package codeforces.round_700_749.round_737;
...
public final class C {
    ...
    public static void main(String[] args) {
        ...
        for (int test = 0; test < t; test++) {
            ...
            for (int i = 0; i < n; i += 2) {
                single = (single + comb.nck(n, i)) % MOD;
            }
            long res = comb.modPow(single, k);
            if (n % 2 == 0) {
                long equal = 1;
                for (int i = 0; i < k; i++) {
                    final long greater = comb.modPow(2, n);
                    long add = comb.modPow(greater, k - 1 - i);
                    add = (add * equal) % MOD;
                    res = (res + add) % MOD;
                    equal = (equal * single) % MOD;
                }
            }
