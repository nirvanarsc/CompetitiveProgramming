...
public final class MilkPails {

    public static void main(String[] args) throws IOException {
        final Kattio fs = new Kattio("pails");
        final int x = fs.nextInt();
        final int y = fs.nextInt();
        final int m = fs.nextInt();
        int res = 0;
        for (int takeX = 0; takeX <= m / x; takeX++) {
            res = Math.max(res, takeX * x + y * ((m - takeX * x) / y));
        }
        fs.println(res);
        fs.close();
    }
