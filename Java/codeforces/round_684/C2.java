...
public final class C2 {
    ...
    private static void shiftLeft(int[][] grid, int row, int col, int[] count, StringBuilder pw) {
        final int upL = grid[row][col];
        final int downL = grid[row + 1][col];
        if (upL == 1 && downL == 0) {
            update(grid, row, col, row, col + 1, row + 1, col + 1, count, pw);
        } else if (upL == 0 && downL == 1) {
            update(grid, row + 1, col, row, col + 1, row + 1, col + 1, count, pw);
        } else if (upL == 1 && downL == 1) {
            update(grid, row, col, row + 1, col, row + 1, col + 1, count, pw);
        }
    }

    private static void shiftDown(int[][] grid, int row, int col, int[] count, StringBuilder pw) {
        final int upL = grid[row][col];
        final int upR = grid[row][col + 1];
        if (upL == 1 && upR == 0) {
            update(grid, row, col, row + 1, col, row + 1, col + 1, count, pw);
    ...
    public static void main(String[] args) {
        ...
        for (int test = 0; test < t; test++) {
            ...
            for (int i = 0; i < n; i += 2) {
                ...
                for (int j = 0; j < m - 2; j++) {
                    shiftLeft(grid, i, j, count, sb);
                }
            }
            for (int i = 0; i < n - 2; i++) {
                shiftDown(grid, i, m - 2, count, sb);
            }
            f(grid, n - 2, m - 2, count, sb);
