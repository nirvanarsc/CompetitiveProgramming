...
public final class G {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final SegTree l = new SegTree(0, n - 1, ll);
        final SegTree r = new SegTree(0, n - 1, rr);
        for (int i = 0; i < q; i++) {
            final int t = fs.nextInt();
            final int u = fs.nextInt() - 1;
            final int v = fs.nextInt() - 1;
            final int smaller = (int) l.query(u, v);
            final int greater = (int) r.query(u, v);
            l.update(u, v, 0, 0);
            r.update(u, v, 0, 0);
            if (t == 1) {
                l.update(u, u + smaller - 1, 0, 1);
                r.update(v - greater + 1, v, 0, 1);
                if (u <= res && res <= v) {
                    res = u + smaller;
                }
            } else {
                l.update(v - smaller + 1, v, 0, 1);
                r.update(u, u + greater - 1, 0, 1);
                if (u <= res && res <= v) {
                    res = u + greater;
                }
            }
