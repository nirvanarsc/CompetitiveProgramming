...
public final class D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            final int pi = lowerBound(pre, p + pre[i]);
            if ((pi != n + 1) && pre[pi] == p + pre[i]) {
                final int qi = lowerBound(pre, p + q + pre[i]);
                if ((qi != n + 1) && pre[qi] == p + q + pre[i]) {
                    final int ri = lowerBound(pre, p + q + r + pre[i]);
                    if ((ri != n + 1) && pre[ri] == p + q + r + pre[i]) {
