package atcoder.beginner_200_299.abc_234;
...
public final class F {
    ...
    public static void main(String[] args) throws IOException {
        ...
        long res = 0;
        for (int i = 1; i <= n; i++) {
            final long add = (comb.factorial[i] * dfs(0, i)) % MOD;
            res = (res + add) % MOD;
        }
        System.out.println(res);
    }

    private static long dfs(int idx, int c) {
        if (idx == 26) {
            return c == 0 ? 1 : 0;
        }
        if (seen[idx][c]) {
        ...
        long res = 0;
        for (int i = 0; i <= Math.min(c, f[idx]); i++) {
            final long add = (comb.facInverse[i] * dfs(idx + 1, c - i)) % MOD;
            res = (res + add) % MOD;
        }
        seen[idx][c] = true;
        return dp[idx][c] = res;
