...
public final class D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final int last = l > r ? 1 : 0;
        final int lastMax = Math.max(l, r);
        for (int i = 0; i < n; i++) {
            if (Character.isUpperCase(w[i]) && lastMax > change[i] && last == 1) {
                w[i] = Character.toLowerCase(w[i]);
            } else if (Character.isLowerCase(w[i]) && lastMax > change[i] && last == 0) {
