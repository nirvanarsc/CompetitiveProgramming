...
public final class TreeMatching {
    ...
    private static void dfs(int u, int v, int[][] g, int[][] dp) {
        int best = (int) 1e9;
        for (int next : g[u]) {
            if (next != v) {
                dfs(next, u, g, dp);
                dp[u][0] += dp[next][1];
                best = Math.min(best, dp[next][1] - dp[next][0]);
            }
        }
        dp[u][1] = Math.max(dp[u][0], dp[u][0] - best + 1);
    }
