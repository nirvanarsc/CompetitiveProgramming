...
public final class ForestQueries {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            g[i] = fs.readLine().toCharArray();
        }
        final int[][] dp = new int[n + 1][n + 1];
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                dp[i][j] = (g[i - 1][j - 1] == '*' ? 1 : 0) + dp[i - 1][j] + dp[i][j - 1] - dp[i - 1][j - 1];
            }
        }
        for (int i = 0; i < q; i++) {
            ...
            sb.append(dp[l2][r2] - dp[l1][r2] - dp[l2][r1] + dp[l1][r1]);
            sb.append('\n');
        }
